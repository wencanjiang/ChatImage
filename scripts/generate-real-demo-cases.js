...
const CASES = [
  ...
  },
  {
    ...
    category: "scene",
    extraInstruction:
      "Use exactly six clickable modules with these exact titles: White Statue; Stone Pedestal; Front Lawn; Main Building; Left Tree Canopy; Right Tree Canopy. Follow a reference-style wide campus composition: a wide horizontal campus view at dusk, pink-purple sunset sky, dark green Laohe Mountain ridge in the far background, a low academic building facade below the ridge, a white standing Mao Zedong statue centered on a tall stone pedestal, trees framing both sides, a broad uninterrupted dark-green front lawn, and a quiet paved plaza around the statue. Create one refined painterly campus scene, but design the six clickable targets as clean separated silhouettes so SAM can segment them: the statue is one continuous white figure with no internal cutout holes; the pedestal is one solid stone block; the front lawn is one continuous simple green foreground shape; the main building is one broad rectangular warm-toned facade; the left tree canopy is one single dark-green blob; the right tree canopy is one separate dark-green blob. Separate these six shapes with visible sky, paving, or grass gaps. Keep the mountain and sunset as non-clickable background context. Do not create a campus map, flowchart, card grid, numbered pins, side legend, visible labels, foreground people, slogans, memorial text, invented building names, exact dates, or West Lake landmarks.",
    question:
