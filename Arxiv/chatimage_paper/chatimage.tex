...
\documentclass[numbers]{april_aigc}
...
\abstract{
Large Language Models (LLMs) can answer complex questions, but the answer is
often returned as dense linear text that is useful but difficult to navigate.
\method{} turns that answer into an \emph{interactive visual image}. The system
first asks an LLM for a normal long-form answer, breaks it into visual modules,
plans an image, and generates the picture. It then analyzes the rendered image
with vision grounding models such as LocateAnything and MiMo-Vision, optionally
refines the regions with SAM-style masks, and places transparent clickable
hotspots on the content that actually appeared. Each hotspot opens a detail
panel and a follow-up thread scoped to that region, allowing the user to inspect
one part of the answer without rereading the full response. The system therefore
treats planned coordinates as priors rather than final interaction geometry:
it generates first, then grounds the visible result. We release an open-source
reference system, assemble a 30-question benchmark across infographic, map, and
scene modes, and evaluate the real-provider pipeline with completion checks, a
strict visual-alignment gate, and a SAM mask-completeness diagnostic. Code,
demos, and a live project page are publicly available.
}
