...
\begin{figure*}[t!]
    ...
    follow-up threads. \tbd{replace with ChatImage architecture figure}}
...
control---which diffusion models do not reliably provide~\citep{feng2024layoutgpt}
...
\paragraph{LocateAnything grounding (primary).}
A visual grounding model (LocateAnything~\citep{wang2025locateanything}) takes
the image and, for each module, one or more locator phrases built from the
module's title, region prompt, and semantic hints. It outputs a bounding box
per module in normalized coordinates, scored against the planned bounds to
...
``where did the module labeled $X$ actually render,'' not detecting a
predefined object class.

\paragraph{Fallbacks.}
When LocateAnything is unavailable, \method{} falls back to a general
vision-language model for grounding~\citep{liu2024internvl}, then to local
OCR, and finally to the planned bounds. Each module is tagged with its
alignment source (e.g., \textit{vision}, \textit{planned-fallback},
...
For tighter visual previews, a segmentation model (SAM3, following the SAM
family~\citep{kirillov2023sam,ravi2024sam2}) takes each grounded box as a
prompt and predicts a pixel mask, from which a tightened box and polygon are
derived. The mask is used only for preview rendering (cutout, organic
